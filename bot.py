#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Bot v17.20 — security hardening + all fixes"""
import sys
import os
import io
import json
...
# ============================================
# КОНСТАНТЫ (из конфига где возможно)
# ============================================
...
BOT_START = time.time()
TOKEN_TTL_SECONDS = 30 * 24 * 60 * 60  # 30 дней
RATE_LIMIT_WINDOW = 60  # секунд
MAX_LOGIN_ATTEMPTS = 5

# Глобальные потокобезопасные структуры
...
if not PASSWORD or PASSWORD == "admin123":
    print("FATAL: weak or default password in config.json", flush=True)
    sys.exit(108)
...
def signal_handler(sig, frame):
    print("[Shutdown] received signal, stopping...", flush=True)
    try:
        bot.stop_polling()
    except:
        pass
...
def wj(p, d):
    ...
    try:
        ...
        print(f"[wj] error writing {p}: {e}", flush=True)
...
def log_failed_login(key, reason):
    try:
        ...
        attempts.append({
            'time': datetime.now(MSK).isoformat(),
            'key': key,
            'reason': reason
        })
        # ограничиваем размер
...
def post_url_to_channel(url, reason="new", retries=5):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"[Tunnel] post attempt {attempt+1}: {e}", flush=True)
            time.sleep(3)
    return False
                    ...
                    if nu != ou:
                        ...
                        try:
                            with open(TUNNEL, 'w', encoding='utf-8') as f:
                                f.write(nu)
                            save_tunnel_state({'last_url': nu})
                        except:
                            pass
                        fails = 0
            p.wait()
            fails += 1
            time.sleep(min(1 + fails, 10))
        except Exception as e:
...
def save_player_history(name, line, important):
    """APPEND-ONLY для производительности"""
    ...
    with players_lock:
        for p in players_in_update:
            ...
            save_player_history(name, format_history_line(
                ...
                "standing", in_tab, vanish, vanish, online_sec, gamemode), vanish)
...
def vanish_checker_loop():
    while True:
        try:
            ...
            print(f"[Vanish] err: {e}", flush=True)
        time.sleep(5)
...
def get_tunnel_status_text():
    with tunnel_health_lock:
        s = tunnel_health.get('status', 'unknown')
        url = tunnel_health.get('url') or tunnel() or 'не настроен'
    if s == 'ok':
...
def parse_site_status():
    ...
    try:
        ...
        players_list = []
        # ИСПРАВЛЕНО v17.20: корректный regex
...
# ============================================
def build_help_text():
    return (f"{ui_header('Справка v17.20','📖')}\n\n"
            f"<b>🚀 Команды:</b>\n"
            f"<code>/start</code> /start\n"
            f"<code>/menu</code> /menu\n"
            f"<code>/status</code> /status\n"
            f"<code>/api</code> /api\n"
            f"<code>/past</code> /past\n"
            f"<code>/all</code> /all")
...
def build_status_text():
    ...
    txt = (f"{ui_header('Статус сервера','🌐')}\n\n{state}\n"
           f"📡 <b>Адрес:</b> <code>{safe(status.get('address','gmd.capscraft.com'))}</code>\n")
...
def status_auto_refresh_loop():
    while True:
        try:
            ...
            print(f"[Status] err: {e}", flush=True)
...
def bpk(ps, pg):
    ...
    for i, p in enumerate(it):
        idx = st_ + i
        kb.add(types.InlineKeyboardButton(f"{idx+1}. 📄 {safe(tr(p['name'], MAX_PN))}",
...
def buk(ud, pg):
    ...
    for i, (uk, d) in enumerate(ip):
        ...
        extra = ""
        if d.get('is_bot'):
            extra = f" 📋{len(d.get('assigned_pastes', []))}"
        kb.add(types.InlineKeyboardButton(f"{i+1}. {ic} {safe(n)}{extra}",
...
def bbpk(uk):
    u = lu().get(uk, {})
    m = u.get('mode', 'normal')
    hbi = u.get('heartbeat_interval', 30)  # ИСПРАВЛЕНО v17.20: по умолчанию 30
    kb = types.InlineKeyboardMarkup(row_width=2)
    if u.get('is_bot'):
        kb.add(types.InlineKeyboardButton("🔧 Режим" if m != 'service' else "🔓 Режим",
                                          callback_data="mode_toggle:" + uk))
        kb.add(types.InlineKeyboardButton("🚫 Кикнуть", callback_data="kick_bot:" + uk))
        kb.add(types.InlineKeyboardButton(f"💓 {hb_category(hbi)}",
...
def build_api_text():
    ...
    # ИСПРАВЛЕНО v17.20: пароль маскируется
...
def show_paste_profile(c, idx):
    ...
    kb.add(types.InlineKeyboardButton("🗑 Удалить", callback_data=f"paste_del:{idx}"),
           types.InlineKeyboardButton("🔙", callback_data="menu:past"))
    try:
        if c.message:
            bot.edit_message_text(f"{ui_header(p['name'],'📄')}\n{ui_row('Размер',f'{len(c_)} байт')}",
...
ADMIN_COMMANDS = PUBLIC_COMMANDS + [
    types.BotCommand("status", "🌐 Статус (live)"),
    types.BotCommand("api", "🖥 API"),
    types.BotCommand("api_reload", "🔄 Рестарт туннеля"),
    types.BotCommand("log", "📄 Логи"),
    types.BotCommand("log_clear", "🗑 Очистить"),
]
...
def cmd_log(m):
    ...
    if not log_content:
        bot.send_message(m.chat.id, "❌ Лог пуст")
        return
    fo = io.BytesIO(log_content.encode('utf-8'))
    fo.name = "bot_log.log"
...
def cmd_log_clear(m):
    if not ia(m.from_user.id):
        ...
        return
    clear_log()
    bot.send_message(m.chat.id, "✅ Лог очищен", parse_mode='HTML')
...
def cmd_api(m):
    ...
    kb.add(types.InlineKeyboardButton("🔙", callback_data="menu:main"))
...
def cmd_past(m):
    ...
    if not pa:
        ps = lp()
        kb, _, _ = bpk(ps, 0)
        bot.send_message(m.chat.id, "📋 Пасты", parse_mode='HTML', reply_markup=kb)
        return
    s = pa[0].lower()
    if s == 'add' and len(pa) >= 2:
        n = tr(pa[1], MAX_PN).lower()
        if any(p['name'].lower() == n for p in lp()):
            bot.send_message(m.chat.id, "⚠️ Уже есть")
            return
        if len(pa) >= 3:
            c = ' '.join(pa[2:])
            e = enc(c)
            ...
            sp(ps)
            bot.send_message(m.chat.id, f"✅ Паст {safe(n)} создан")
            return
        sets(m.from_user.id, {'step': 'add_file_wait', 'paste_name': n})
        bot.send_message(m.chat.id, "📄 Отправьте текст/файл или /cancel")
        return
    if s == 'edit' and len(pa) >= 2:
        ...
        bot.send_message(m.chat.id, "✏️ Отправьте текст/файл или /cancel")
        return
    if s == 'delete' and len(pa) >= 2:
        ...
        bot.send_message(m.chat.id, f"⚠️ Удалить {safe(paste['name'])}?",
...
def cmd_all(m):
    ...
    if not pa:
        kb, _, _ = buk(us, 0)
        bot.send_message(m.chat.id, "👥 Компьютеры", parse_mode='HTML', reply_markup=kb)
        return
    s = pa[0].lower()
    if s == 'assign' and len(pa) >= 3 and ia(m.from_user.id):
        tid, td = find_user_by_arg(pa[1], list(us.items()))
        if not tid or not td.get('is_bot'):
            bot.send_message(m.chat.id, "❌")
            return
        pn = pa[2].lower()
        ...
        if pn not in cp:
            cp.append(pn)
            us[tid]['assigned_pastes'] = cp
            su(us)
        bot.send_message(m.chat.id, f"✅ Привязан {safe(pn)}")
        return
    if s == 'unassign' and len(pa) >= 2 and ia(m.from_user.id):
        ...
        if tid:
            ...
            su(us)
            bot.send_message(m.chat.id, "✅ Отвязано")
        return
    if s == 'kick' and len(pa) >= 2 and ia(m.from_user.id):
        tid, td = find_user_by_arg(pa[1], list(us.items()))
        if tid:
            bot.send_message(m.chat.id, f"⚠️ Кикнуть {safe(td.get('name'))}?",
                            reply_markup=confirm_keyboard(f"kick:{tid}"))
        return
...
def handle_document(m):
    ...
    if step not in ['add_file_wait', 'edit_file_wait']:
        return
    if m.document.file_size > MAX_CONTENT_LENGTH:
        bot.send_message(m.chat.id, f"❌ Файл больше {MAX_CONTENT_LENGTH//1024//1024}MB")
        return
    try:
    ...
    if step == 'add_file_wait':
        ...
        sp(ps)
        cs(m.from_user.id)
        bot.send_message(m.chat.id, f"✅ Паст {safe(name)} создан")
    elif step == 'edit_file_wait':
        idx = state.get('idx')
        ps = lp()
        if 0 <= idx < len(ps):
            ps[idx]['content'] = enc(content)
            ps[idx]['hash'] = chash(content)
            ps[idx]['edited_at'] = datetime.now(MSK).isoformat()
            sp(ps)
        cs(m.from_user.id)
        bot.send_message(m.chat.id, "✅ Обновлён")
...
def cb(c):
    try:
        ...
        if d.startswith("menu:"):
            sec = d.split(":")[1]
            if sec == "main":
                bot.edit_message_text("📱 Меню", c.message.chat.id, c.message.message_id,
                                     reply_markup=main_menu_keyboard(c.from_user.id))
            elif sec == "past":
                kb, _, _ = bpk(lp(), 0)
                bot.edit_message_text("📋 Пасты", c.message.chat.id, c.message.message_id, reply_markup=kb)
            elif sec == "all":
                kb, _, _ = buk(lu(), 0)
                bot.edit_message_text("👥 Компьютеры", c.message.chat.id, c.message.message_id, reply_markup=kb)
            elif sec == "status":
                txt = build_status_text()
                if txt:
                    bot.edit_message_text(txt, c.message.chat.id, c.message.message_id,
                ...
                kb = types.InlineKeyboardMarkup(row_width=2)
                kb.add(types.InlineKeyboardButton("🔄 Рестарт", callback_data="reload_tunnel"))
                bot.edit_message_text(build_api_text(), c.message.chat.id, c.message.message_id,
                                     parse_mode='HTML', reply_markup=kb)
            elif sec == "help":
                bot.edit_message_text(build_help_text(), c.message.chat.id, c.message.message_id,
        ...
        if d.startswith("mode_toggle:"):
            ...
            if uk in us and us[uk].get('is_bot'):
                us[uk]['mode'] = 'service' if us[uk].get('mode') != 'service' else 'normal'
                su(us)
            sbp(c.message.chat.id, c.message.message_id, uk)
            bot.answer_callback_query(c.id)
            return
        if d.startswith("kick_bot:"):
            ...
            if not ia(c.from_user.id):
                return
            bot.edit_message_text("⚠️ Кикнуть?", c.message.chat.id, c.message.message_id,
...
def sbp(cid, mid, uk):
    ...
    txt = (f"{ui_header(u.get('name',''),'🤖')}\n"
           ...
           f"{ui_row('Скрипты', len(u.get('assigned_pastes', [])))}")
    try:
...
def hm(m):
    ...
    if s:
        ...
        if stp == 'wp':
            ...
            if t == PASSWORD:
                ...
                us[str(u)] = {
                    ...
                    'is_bot': s.get('is_bot'), 'is_admin': False,
                    'registered_at': datetime.now(MSK).isoformat()
                }
                su(us)
                cs(u)
        ...
        if stp == 'add_file_wait':
            ...
            sp(ps)
            cs(u)
            bot.send_message(m.chat.id, f"✅ {safe(name)} создан")
            return
...
class AH(BaseHTTPRequestHandler):
    ...
    def _j(self, c, d):
        try:
            ...
            # CORS: только разрешённые домены или ничего
            if ALLOWED_ORIGINS:
    ...
    def _check_friend_auth(self):
        ...
        if not k or k != PASSWORD:
            return False
        # ИСПРАВЛЕНО v17.20: не используем X-Forwarded-For (можно подделать)
    ...
    def _get(self):
        ...
        if p == '/api/health':
            ...
            return
        if p == '/api/reload':
            # ИСПРАВЛЕНО v17.20: требует токен
            if not tok:
        ...
        if p.startswith('/api/player/'):
            if not self._check_friend_auth():
                ...
                return
            name = p.split('/')[-1]
            # ИСПРАВЛЕНО v17.20: валидация имени
            ...
            if not fp.exists():
                ...
                return
            with open(fp, 'r', encoding='utf-8') as f:
                self._j(200, {"name": name, "history": f.read()})
            return
        if p == '/api/me':
            if not tok:
            ...
            self._j(200, {
                "ok": True, "computer_id": td.get('computer_id'),
                "role": "bot" if ib else "human",
                "assigned_pastes": up, "mode": um,
        ...
        if p.startswith('/api/paste/'):
            ...
            if ib:
                ...
                if not al or n not in al:
                    self._j(403, {"error": "PANIC"})
                    return
            for x in lp():
                if x['name'].lower() == n:
                    c = dec(x['content'])
                    self._j(200, {"name": x['name'], "content": c})
                    return
            self._j(404, {"error": "no"})
            return
        self._j(404, {"error": "no"})
        ...
        if p == '/api/login':
            ...
            for uid, ud in us.items():
                if ud.get('is_bot') and ud.get('computer_id') == lci:
                    et = ud.get('api_token')
                    if et and et in ts:
                        self._j(200, {"ok": True, "status": "already_registered", "token": et})
                        return
            pid = str(uuid.uuid4())
            ft = str(uuid.uuid4())
            pe = lpend()
            pe[pid] = {
                'token': ft, 'name': d.get('name'),
                'computer_id': lci, 'status': 'pending'
            }
            spend(pe)
            # авто-одобрение
            ts[ft] = {
                'name': d.get('name'), 'computer_id': lci,
...
def start_api():
    while True:
        try:
            ...
            srv.timeout = 5
            print(f"[API] Ready v17.20 on {PORT}", flush=True)
            srv.serve_forever()
        except OSError as e:
            print(f"[API] OSError: {e}", flush=True)
            try:
...
def main():
    print("Starting bot v17.20...", flush=True)
