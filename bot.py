#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Bot v17.25 — Cloudflared PRIMARY + 5G survivability + all fixes"""
...
# Доверенные игроки (Telegram ID -> игровой ник)
TRUSTED_PLAYERS = {
    5183248850: "Gishta1",
    5602435561: "Rainy42",
...
def aia():
    ...
    for s, d in lu().items():
        try:
            ...
            pass
    for uid in TRUSTED_PLAYERS.keys():
...
internet_state = {'online': True, 'down_since': None}
        ...
        for line in lines:
            ...
            try:
                ...
                if len(parts) >= 2:
                    ...
                    ttype = parts[3] if len(parts) > 3 else 'cloudflared'
...
def find_cloudflared():
    """Находит cloudflared: PATH -> локальный бинарь -> pkg -> скачивание."""
    global CLOUDFLARED_CMD
    if CLOUDFLARED_CMD:
        return CLOUDFLARED_CMD
    try:
        r = subprocess.run("which cloudflared", shell=True, capture_output=True)
        if r.returncode == 0:
    ...
    if local.exists() and os.access(str(local), os.X_OK):
        CLOUDFLARED_CMD = str(local)
        return CLOUDFLARED_CMD
    try:
        print("[Tunnel] пытаюсь установить cloudflared через pkg...", flush=True)
        subprocess.run("pkg install -y cloudflared", shell=True, capture_output=True, timeout=180)
        r = subprocess.run("which cloudflared", shell=True, capture_output=True)
        if r.returncode == 0:
            CLOUDFLARED_CMD = "cloudflared"
            return CLOUDFLARED_CMD
    except Exception as e:
        print(f"[Tunnel] pkg install cloudflared failed: {e}", flush=True)
    try:
        ...
        print("[Tunnel] скачиваю бинарь cloudflared...", flush=True)
        subprocess.run(f"curl -sL -o '{local}' '{url}'", shell=True, timeout=180)
        os.chmod(str(local), 0o755)
        if local.exists() and os.access(str(local), os.X_OK):
            CLOUDFLARED_CMD = str(local)
            return CLOUDFLARED_CMD
    except Exception as e:
        print(f"[Tunnel] download cloudflared failed: {e}", flush=True)
    return None

def run_cloudflared_once():
    """Одна сессия cloudflared; возвращает управление когда процесс умер."""
    global tunnel_process, tunnel_type, tunnel_last_activity, tunnel_reconnects
    cmd = find_cloudflared()
    if not cmd:
        return False
    print("[Tunnel] запуск cloudflared (порт 443, не блокируется)...", flush=True)
    tunnel_last_activity = time.time()
    try:
        p = subprocess.Popen(
        ...
        for line in iter(p.stdout.readline, ''):
            ...
            if not line:
                continue
            tunnel_last_activity = time.time()
            if 'trycloudflare.com' in line or 'ERR' in line or 'WRN' in line:
...
def run_ssh_once():
    """Одна SSH-сессия localhost.run (фолбэк)."""
    global tunnel_process, tunnel_type, tunnel_last_activity, tunnel_reconnects
    ...
    print("[Tunnel] запуск localhost.run (фолбэк SSH)...", flush=True)
    tunnel_last_activity = time.time()
    try:
        p = subprocess.Popen(
        ...
        for line in iter(p.stdout.readline, ''):
            ...
            if not line:
                continue
            tunnel_last_activity = time.time()
            if 'lhr.life' in line or 'error' in line.lower() or 'timed out' in line.lower() or 'connection' in line.lower():
...
def tunnel_main_loop():
    """Главный поток туннеля: cloudflared PRIMARY, ssh FALLBACK, учитывает интернет."""
    fails = 0
    while True:
        if not internet_state['online']:
            time.sleep(10)
            continue
        cmd = find_cloudflared()
        if cmd:
            ok = run_cloudflared_once()
        else:
            ok = run_ssh_once()
        if ok:
            fails = 0
            ...
            fails += 1
            wait = min(2 + fails, 15)
            print(f"[Tunnel] сессия упала (fail #{fails}), рестарт через {wait}с...", flush=True)
            time.sleep(wait)
...
def internet_monitor_loop():
    """Монитор интернета (5G): пинг, при восстановлении — рестарт туннеля."""
    while True:
        time.sleep(20)
        ok = False
        for host in ("1.1.1.1", "8.8.8.8"):
            try:
                r = subprocess.run(["ping", "-c", "1", "-W", "3", host], capture_output=True, timeout=6)
                if r.returncode == 0:
                    ok = True
                    break
            except Exception:
                continue
        was_online = internet_state['online']
        internet_state['online'] = ok
        if ok and not was_online:
            print("[NetMon] 🟢 интернет восстановлен → перезапуск туннеля", flush=True)
            internet_state['down_since'] = None
            force_reload_tunnel("internet_restored")
        elif not ok and was_online:
            internet_state['down_since'] = time.time()
            print("[NetMon] 🔴 интернет пропал — жду восстановления", flush=True)

def tunnel_health_watchdog():
    """Реальный health-check публичного URL; 3 фейла подряд → рестарт."""
    fail_streak = 0
    while True:
        time.sleep(30)
        ...
        try:
            req = urllib.request.Request(url + "/api/health",
                headers={'bypass-tunnel-reminder': 'true', 'User-Agent': 'WD/1.0'})
        ...
        if alive:
            ...
            fail_streak += 1
            print(f"[WD] туннель не отвечает ({fail_streak}/3)", flush=True)
            if fail_streak >= 3:
                force_reload_tunnel("health_watchdog")
                fail_streak = 0

# ============================================
# TUNNEL HEALTH (для /api)
# ============================================
tunnel_health = {
    'status': 'unknown', 'url': None, 'type': None, 'last_check': None,
    'last_ok': None, 'errors': [], 'checks_total': 0, 'checks_ok': 0, 'last_error': None
}
...
def check_tunnel_health():
    ...
    try:
        ...
        print(f"[TunnelHealth] err: {e}", flush=True)
    ...
    try:
        ...
        if s == 'ok':
            ...
            return (f"🟢 <b>Работает</b>{tinfo}\n"
                    f"{ui_row('Успех', f'{rate}%')}\n"
                    f"{ui_row('URL', url)}\n"
                    f"{ui_row('Переподключений', tunnel_reconnects)}")
        elif s == 'tunnel_down':
            return (f"🟡 <b>Локально работает</b>{tinfo}\n"
                    f"{ui_row('URL', url)}\n"
                    f"{ui_row('Статус', 'Проверяется...')}")
        elif s == 'bot_down':
...
def bbpk(uk):
    ...
    if u.get('is_bot'):
        ...
        if u.get('kiktime_override'):
            kb.add(types.InlineKeyboardButton(f"⏱ Сбросить ({u['kiktime_override']}м)", callback_data="kt_reset:" + uk))
        else:
...
def build_api_text():
    ...
    return (f"{ui_header('API Информация', '🖥')}\n\n"
            ...
            f"<b>🔌 Подключение:</b>\n"
            f"{ui_row('Тип', '☁️ cloudflared' if ttype == 'cloudflared' else ('🔴 ssh' if ttype == 'ssh' else '🏠 Локальная сеть'))}\n"
            f"{ui_row('URL', su_)}\n"
...
def cmd_api_reload(m):
    ...
    bot.send_message(m.chat.id, "🔄 <b>Перезагрузка туннеля...</b>\n\nНовый URL появится в канале через <b>2-5 секунд</b>.", parse_mode='HTML')
    ...
    if s == 'add':
        if len(pa) == 2:
            ...
            if not e:
                ...
                return
            ps = lp()
            ps.append({'name': n, 'content': e, 'hash': chash(c), 'cid': m.from_user.id, 'cn': dn(m.from_user.id), 'created_at': datetime.now(MSK).isoformat()})
            sp(ps)
...
def handle_document(m):
    ...
    if step == 'add_file_wait':
        ...
        if not e:
            ...
            return
        ps = lp()
        ps.append({'name': name, 'content': e, 'hash': chash(content), 'cid': m.from_user.id, 'cn': dn(m.from_user.id), 'created_at': datetime.now(MSK).isoformat()})
        sp(ps)
        cs(m.from_user.id)
...
def cb(c):
    try:
        ...
        if d == "reload_tunnel":
            ...
            try:
                bot.send_message(c.message.chat.id, "🔄 <b>Перезагрузка туннеля...</b>\n\nНовый URL появится в канале через <b>2-5 секунд</b>.", parse_mode='HTML')
            except Exception:
                pass
            return
        if d == "toggle_vanish":
            ...
            try:
                bot.edit_message_text(txt, c.message.chat.id, c.message.message_id, parse_mode='HTML', reply_markup=main_menu_keyboard(c.from_user.id))
            except Exception:
                pass
            return
        ...
        if d.startswith("confirm:"):
            ...
            if aid.startswith("del_paste:"):
                ...
                if 0 <= idx < len(ps):
                    ...
                    delete_last_file(c.from_user.id)
                    try:
                        bot.edit_message_text(f"✅ <b>Удалён:</b> <code>{safe(rm['name'])}</code>", c.message.chat.id, c.message.message_id, parse_mode='HTML')
                    except Exception:
                        pass
                ...
                if tid in us:
                    ...
                    cs(c.from_user.id)
                    try:
                        bot.edit_message_text(f"🚫 <b>Кикнут:</b> {safe(tn)}", c.message.chat.id, c.message.message_id, parse_mode='HTML')
                    except Exception:
                        pass
        ...
        if d.startswith("aa:"):
            ...
            cid = p[pid].get('cid', '?')
            ts = lt()
            ts[tok] = {'name': nm, 'computer_id': cid, 'created_at': datetime.now(MSK).isoformat(), 'approved_by': c.from_user.id, 'is_computer': True, 'pending_id': pid}
            st(ts)
            us = lu()
            us[pid] = {'name': nm, 'computer_id': cid, 'username': "pc_" + pid[:8], 'is_bot': True, 'is_admin': False, 'mode': 'normal', 'assigned_pastes': [], 'registered_at': datetime.now(MSK).isoformat(), 'api_token': tok, 'heartbeat_interval': 30}
            su(us)
    ...
    if s:
        ...
        if stp == 'wp':
            ...
            if t == PASSWORD:
                if s.get('is_bot'):
                    us = lu()
                    us[str(u)] = {'name': None, 'username': s.get('username'), 'is_bot': True, 'is_admin': False, 'registered_at': datetime.now(MSK).isoformat()}
                    su(us)
                    cs(u)
        ...
        if stp == 'wn':
            ...
            us[str(u)] = {'name': n, 'username': s.get('username'), 'is_bot': False, 'is_admin': aa, 'registered_at': datetime.now(MSK).isoformat()}
            su(us)
            cs(u)
        ...
        if stp == 'add_file_wait':
            ...
            if not e:
                ...
                return
            ps = lp()
            ps.append({'name': name, 'content': e, 'hash': chash(t), 'cid': u, 'cn': dn(u), 'created_at': datetime.now(MSK).isoformat()})
            sp(ps)
            cs(u)
        ...
        if stp == 'edit_file_wait':
            ...
            sp(ps)
            cs(u)
            bot.send_message(m.chat.id, f"{ui_header('Паст обновлён', '✅')}\n\n{ui_row('Имя', ps[idx]['name'])}", parse_mode='HTML')
            return
        if stp == 'dc':
            idx = s.get('idx')
...
class AH(BaseHTTPRequestHandler):
    ...
    def _get(self):
        ...
        if p == '/api/health':
            ...
            with tunnel_lock:
                h = dict(tunnel_health)
            h.update(bot_status='running', bot_version='17.25',
                     ...
                     total_pastes=len(lp()),
                     tunnel_type=get_current_tunnel_type(),
        ...
        if p == '/api/relay_url':
            u = get_current_tunnel_url()
            if u:
                self._j(200, {"url": u, "timestamp": datetime.now(MSK).isoformat(), "channel": f"https://t.me/s/{CHANNEL_USERNAME}", "relay": "telegram"})
            else:
        ...
        if p == '/api/pastes':
            if ib:
                ...
                return
            ps = lp()
            self._j(200, {"pastes": [{"name": x['name'], "creator": x.get('cn', 'API'), "hash": x.get('hash')} for x in ps], "count": len(ps)})
            return
        self._j(404, {"error": "no"})
    ...
    def _post(self):
        ...
        if p == '/api/login':
            ...
            st(ts)
            us[pid] = {'name': n, 'computer_id': lci, 'username': "pc_" + pid[:8], 'is_bot': True, 'is_admin': False, 'mode': 'normal', 'assigned_pastes': [], 'registered_at': datetime.now(MSK).isoformat(), 'api_token': ft, 'heartbeat_interval': 30}
            su(us)
...
def main():
    print("Starting bot v17.25 (cloudflared primary + 5G survivability)...", flush=True)
